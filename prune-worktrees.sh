...
#   5. the T3 thread that owns it is settled, archived, or gone from the T3 database.
#
# Settled threads count as finished: auto-settle parks idle and merged work there
# without ever archiving it, so most old worktrees belong to settled threads. If
# such a thread is un-settled later, the server recreates its worktree from the
# branch (ignored files like node_modules are gone and need reinstalling).
#
...
# Worktree paths owned by a thread that is still live (not settled, archived, or deleted).
