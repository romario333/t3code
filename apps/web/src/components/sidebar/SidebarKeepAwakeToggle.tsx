 ...
 * server holds a `caffeinate` process so the host Mac does not sleep during
 * unattended agent work. The display is still allowed to sleep. Gated on the primary server's platform (not the
...
export function SidebarKeepAwakeToggle() {
  ...
  const tooltip = keepAwake
    ? "This Mac is being kept awake so agents can keep working. Click to allow sleep again."
    : "Prevent this Mac from sleeping while the T3 Code server is running (uses caffeinate). The display can still sleep.";
