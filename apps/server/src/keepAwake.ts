 ...
 * sleep while on AC power. `-w` ties the assertion to the server process so
 * caffeinate exits on its own if the server dies without running finalizers
 * (a plain kill would otherwise orphan it and pin the machine awake).
 */
